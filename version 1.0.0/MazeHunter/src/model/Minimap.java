...
public class Minimap {
    ...
    // --- NEW: Data structure for "memory" ---
    // This 2D array will store the System.currentTimeMillis() when a tile was last seen.
    private long[][] revealTimestamps;
    private final long revealDurationMillis = 4000; // 4 seconds
    ...
    public Minimap(double size, String shape, int mazeWidth, int mazeHeight) {
        ...
        // --- NEW: Initialize the memory grid ---
        // We initialize all timestamps to 0, meaning they have never been seen.
     ...
     //Redraws the minimap based on the current game state and memory.
    ...
    public void update(int[][] mazeGrid, int playerMazeX, int playerMazeY, int playerVisionRadius, int radius) {
        ...
        if (temporaryBeamPath != null) {
            // --- THIS IS THE FIX ---
            // Calculate the top-left maze coordinate of the player's MAIN GAME view.
            // We MUST use the mainGameRadius for this conversion.
        ...
        for (int row = 0; row < mazeHeight; row++) {
            for (int col = 0; col < mazeWidth; col++) {
                ...
                if (currentTime - lastSeenTime > revealDurationMillis) {
                    // This tile was seen too long ago. Mark it as unseen again.
                    revealTimestamps[row][col] = 0;
                    continue; // Skip drawing, leaving it as fog.
                }

                // If we reach here, the tile is currently revealed.
                int cellType = mazeGrid[row][col];
                switch (cellType) {
                    case 1: gc.setFill(pathColor); break;
                    case -2: gc.setFill(exitColor); break;
                    default: gc.setFill(wallColor); break;
                }
        ...
        gc.fillOval(playerMapX - markerSize / 2 + cellWidth / 2, playerMapY - markerSize / 2 + cellHeight / 2, markerSize, markerSize);
        gc.strokeOval(playerMapX - markerSize / 2 + cellWidth / 2, playerMapY - markerSize / 2 + cellHeight / 2, markerSize, markerSize);
        
        if ("circle".equalsIgnoreCase(shape)) {
            gc.setStroke(Color.WHITE);
            gc.setLineWidth(4);  // thickness of your border
